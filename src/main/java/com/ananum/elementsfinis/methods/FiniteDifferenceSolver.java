...
public class FiniteDifferenceSolver {
    ...
    public FiniteDifferenceSolver(int n, int m) {
        this.n = n;
        this.m = m;
        this.h = 1.0 / (n + 1);
        this.k = 1.0/ (m + 1);
    ...
    public double[] solve() {
        ...
        for (int j = 0; j < m; j++) {
            for (int i = 0; i < n; i++) {
                ...
                else       b[idx] += exactSolution((i) * h, (j + 1) * k)*ih; // gauche
                if (i < n - 1) A[idx][idx + 1] = -ih;
                else           b[idx] += exactSolution((i + 2) * h, (j + 1) * k)*ih; // droite
                if (j > 0) A[idx][idx - n] = -ik;
                else       b[idx] += exactSolution((i + 1) * h, (j) * k)*ik; // bas
                if (j < n - 1) A[idx][idx + n] = -ik;
                else           b[idx] += exactSolution((i + 1) * h, (j + 2) * k)*ik; // haut

                b[idx] += fValue((i+1)*h,(j+1)*k); // f(x,y) = -4 donc RHS = -4 * h^2, 
            }
        }
